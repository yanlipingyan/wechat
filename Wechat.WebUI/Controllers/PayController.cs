...
namespace Wechat.WebUI.Controllers
{
    public class PayController : Controller
    {
        ...
        public ActionResult GongZhongHaoPay(string id, string code)
        {
            ...
            {
                var token = Wechat.WebUI.OAuth2.GetToken(Wechat.WebUI.Model.ApiModel.AppID, Wechat.WebUI.Model.ApiModel.AppSecret, code);

                ViewBag.Oauth2Token = JsonConvert.DeserializeObject<dynamic>(token)["access_token"];
                ViewBag.AppId = Wechat.WebUI.Model.ApiModel.AppID;
            ...
            return Redirect(Wechat.WebUI.OAuth2.GetCode(Wechat.WebUI.Model.ApiModel.AppID, Request.Url.AbsoluteUri, "snsapi_base", "state"));
        }
        ...
        public string JsPay(string title, string description, int price)
        {
            ...
            orderParams.Add("appid", Wechat.WebUI.Model.ApiModel.AppID);
            orderParams.Add("attach", title);
            orderParams.Add("body", description);
            orderParams.Add("mch_id", Wechat.WebUI.Model.ApiModel.MchID);
            orderParams.Add("nonce_str", Wechat.WebUI.Common.GetNonceStr());
            ...
            orderParams.Add("out_trade_no", Wechat.WebUI.Pay.GetOutTradeNo(Wechat.WebUI.Model.ApiModel.MchID));
            orderParams.Add("spbill_create_ip", "171.8.215.143");
            orderParams.Add("total_fee", price);
            orderParams.Add("trade_type", "JSAPI");
            orderParams.Add("sign", Wechat.WebUI.Pay.GetSign(orderParams, Wechat.WebUI.Model.ApiModel.MchAPISecret));

            var result = Wechat.WebUI.Pay.UnifiedOrder(orderParams);

            //统一下单失败，返回错误结果给微信平台
            if (result["return_code"].ToString() != "SUCCESS")
                return JsonConvert.SerializeObject(Wechat.WebUI.Pay.ErrorInfo("统一下单失败"));

            //获取H5调起JS API参数
            SortedDictionary<string, object> apiParams = new SortedDictionary<string, object>();

            apiParams.Add("appId", Wechat.WebUI.Model.ApiModel.AppID);
            apiParams.Add("timeStamp", Wechat.WebUI.Common.GetTimeStamp());
            apiParams.Add("nonceStr",Wechat.WebUI.Common.GetNonceStr());
            apiParams.Add("package", "prepay_id=" + result["prepay_id"].ToString());
            apiParams.Add("signType", "MD5");
            apiParams.Add("paySign", Wechat.WebUI.Pay.GetSign(apiParams, Wechat.WebUI.Model.ApiModel.MchAPISecret));
        ...
        {
            SortedDictionary<string, object> orderParams = new SortedDictionary<string, object>();
            orderParams.Add("appid", Wechat.WebUI.Model.ApiModel.AppID);//公众帐号id
            orderParams.Add("mch_id", Wechat.WebUI.Model.ApiModel.MchID);//商户号
            orderParams.Add("time_stamp",Wechat.WebUI.Common.GetTimeStamp());//时间戳
            orderParams.Add("nonce_str",Wechat.WebUI.Common.GetNonceStr());//随机字符串
            orderParams.Add("product_id", id);//商品订单号
            orderParams.Add("sign", Wechat.WebUI.Pay.GetSign(orderParams, Wechat.WebUI.Model.ApiModel.MchAPISecret));//签名

            //预支付URL
            var url = Wechat.WebUI.Pay.GetPayUrlForNativeOne(orderParams);
        ...
        {
            var result = Wechat.WebUI.Pay.GetNotifyData();

            if (string.IsNullOrEmpty(result["openid"].ToString()) || string.IsNullOrEmpty(result["product_id"].ToString()))
                return Wechat.WebUI.Common.SortedDictionaryToXml(Wechat.WebUI.Pay.ErrorInfo("回调数据异常"));
            ...
            orderParams.Add("out_trade_no", Wechat.WebUI.Pay.GetOutTradeNo(Wechat.WebUI.Model.ApiModel.MchID));//result["product_id"].ToString()
            orderParams.Add("spbill_create_ip", "171.8.215.143");
            orderParams.Add("total_fee", 1);
            orderParams.Add("trade_type", "NATIVE");
            orderParams.Add("sign", Wechat.WebUI.Pay.GetSign(orderParams, Wechat.WebUI.Model.ApiModel.MchAPISecret));

            var orderResult = Wechat.WebUI.Pay.UnifiedOrder(orderParams);

            //统一下单失败，返回错误结果给微信平台
            if (orderResult["return_code"].ToString() != "SUCCESS")
                return Wechat.WebUI.Common.SortedDictionaryToXml(Wechat.WebUI.Pay.ErrorInfo("统一下单失败"));
            ...
            resultParams.Add("appid", Wechat.WebUI.Model.ApiModel.AppID);
            resultParams.Add("mch_id", Wechat.WebUI.Model.ApiModel.MchID);
            resultParams.Add("nonce_str",Wechat.WebUI.Common.GetNonceStr());
            ...
            resultParams.Add("sign", Wechat.WebUI.Pay.GetSign(resultParams, Wechat.WebUI.Model.ApiModel.MchAPISecret));

            return Wechat.WebUI.Common.SortedDictionaryToXml(resultParams);
        }
        #endregion
        ...
        public void NativePayTwo(string id)
        {
            ...
            orderParams.Add("out_trade_no", Wechat.WebUI.Pay.GetOutTradeNo(Wechat.WebUI.Model.ApiModel.MchID));//随机字符串
            orderParams.Add("spbill_create_ip", "171.8.215.143");
            ...
            //直接支付URL
            var url = Wechat.WebUI.Pay.GetPayUrlForNativeTwo(orderParams);
        ...
        {
            var result = Wechat.WebUI.Pay.PayResultNotify();

            return Wechat.WebUI.Common.SortedDictionaryToXml(result);
        }
